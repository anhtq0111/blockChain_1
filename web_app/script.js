...
// return array of all the creditors of user and amount owe 
async function getCreditorAndAmount(user){
	const creditors = await getUsers();

	const creditorAndAmount = [];
	for(const creditor of creditors){
		if(user !== creditor){
			const owe = await BlockchainSplitwise.lookup(user, creditor);
			const amt = Number(owe);

			amt && creditorAndAmount.push({
				creditor,
				amt
			})
		}
	...
	return creditorAndAmount;
}

// return array of the address of all creditors of user
async function getNeighbors(user){
	const creditorAndAmount = await getCreditorAndAmount(user);
	return creditorAndAmount.map(({creditor}) => creditor);
}

// TODO: Return a list of all users (creditors or debtors) in the system
// All users in the system are everyone who has ever sent or received an IOU
async function getUsers() {
	const function_calls = await getAllFunctionCalls(contractAddress, 'addIOU');

	const usersSet = function_calls.reduce((users,{from, args}) => {
		const [to, _] = args;
		users.add(from);
		users.add(to);
		return users;
...
async function getTotalOwed(user) {
	const creditorAndAmount = await getCreditorAndAmount(user);
	return creditorAndAmount.reduce((totalOwed, {amt}) => {
		totalOwed += amt;
		return totalOwed;
	}, 0);
}
...
async function getLastActive(user) {
	const function_calls = await getAllFunctionCalls(contractAddress, 'addIOU');
	const condition = (from, args) => from.toLowerCase() === user.toLowerCase() || args[0].toLowerCase() === user.toLowerCase();
	const lastActive = function_calls.find(obj => condition(obj.from, obj.args));
	

	return lastActive ? lastActive.t : null;
}
